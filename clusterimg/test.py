...
generate_test_dataset(test_path, 1000)

clstr = Clustering(images_folder_path=test_path, method="minhash", num_of_threads=8, threshold=0.125,
                   batch_size=50, overwrite="Y", transfer="copy", verbose=6, size=(100,100), option="dontmerge")
print(clstr)
clstr()

clstr = Clustering(images_folder_path=test_path + "_clustered", method="minhash", num_of_threads=8, threshold=0.125,
